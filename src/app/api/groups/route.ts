...
export async function GET(request: NextRequest) {
  try {
    ...
    // Transform the data to include task counts and statistics if requested
    const transformedGroups = groups.map(group => {
      if (includeTaskCount && group.tasks) {
        const totalTasks = group.tasks.length;
        // Use status field if available, fallback to completed field
        const completedTasks = group.tasks.filter(task => 
          task.status ? task.status === 'completed' : task.completed
        ).length;
        const pendingTasks = group.tasks.filter(task => 
          task.status ? task.status === 'pending' : !task.completed
        ).length;
        const ongoingTasks = group.tasks.filter(task => 
          task.status === 'ongoing'
        ).length;
