...
const TaskHistory: React.FC<TaskHistoryProps> = ({ taskId, taskTitle, isVisible, onClose }) => {
  ...
  const formatAction = (activity: ActivityLog) => {
    ...
    // Use subtask title if available, otherwise use task title
    const displayTitle = activity.subtaskTitle || taskTitle;
    const isSubtaskAction = activity.isSubtask || activity.subtaskTitle;

    switch (activity.action.toLowerCase()) {
      case 'created':
        return isSubtaskAction 
          ? `Creó la subtarea "${displayTitle}"`
          : `Creó la tarea "${displayTitle}"`;
      case 'updated':
        return isSubtaskAction 
          ? `Actualizó la subtarea "${displayTitle}"`
          : `Actualizó la tarea "${displayTitle}"`;
      case 'completed':
        return isSubtaskAction 
          ? `Marcó como completada la subtarea "${displayTitle}"`
          : `Marcó como completada "${displayTitle}"`;
      case 'uncompleted':
        return isSubtaskAction 
          ? `Marcó como pendiente la subtarea "${displayTitle}"`
          : `Marcó como pendiente "${displayTitle}"`;
      case 'subtask_added':
        return activity.subtaskTitle 
          ? `Agregó la subtarea "${activity.subtaskTitle}"`
          : 'Agregó una subtarea';
      case 'subtask_updated':
        return activity.subtaskTitle 
          ? `Actualizó la subtarea "${activity.subtaskTitle}"`
          : 'Actualizó una subtarea';
      case 'subtask_completed':
        return activity.subtaskTitle 
          ? `Completó la subtarea "${activity.subtaskTitle}"`
          : 'Completó una subtarea';
      case 'priority_changed':
        return isSubtaskAction 
          ? `Cambió la prioridad de la subtarea "${displayTitle}"`
          : `Cambió la prioridad de "${displayTitle}"`;
      case 'due_date_changed':
        return isSubtaskAction 
          ? `Cambió la fecha de vencimiento de la subtarea "${displayTitle}"`
          : `Cambió la fecha de vencimiento de "${displayTitle}"`;
      case 'tags_updated':
        return isSubtaskAction 
          ? `Actualizó las etiquetas de la subtarea "${displayTitle}"`
          : `Actualizó las etiquetas de "${displayTitle}"`;
      default:
          ...
          <div className="space-y-3" data-comment-form>
            <div className="relative">
              <textarea
                ...
                className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg text-sm resize-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 min-h-[80px] dark:bg-gray-700 dark:text-gray-100 dark:placeholder-gray-400"
                rows={3}
                autoFocus
                maxLength={500}
