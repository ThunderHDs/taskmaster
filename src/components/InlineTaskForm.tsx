...
import { Task, Priority, Tag, Group } from '@/types';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Badge } from '@/components/ui/badge';
import { X, Plus, Calendar, Clock, Tag as TagIcon, Users, Copy, ChevronDown, ChevronUp, AlertCircle } from 'lucide-react';
...
const InlineTaskForm: React.FC<InlineTaskFormProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!startDate && formData.dueDate) {
      ...
      const now = safeNow();
      startDate = safeDate(now.getFullYear() + '-' + (now.getMonth() + 1).toString().padStart(2, '0') + '-' + now.getDate().toString().padStart(2, '0'));
    ...
    <div ref={formRef} className={`bg-white rounded-lg border border-gray-200 ${className}`}>
      {!isExpanded ? (
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
           ...
           */}
          <div>
            <label htmlFor="inline-description" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Description
            </label>
