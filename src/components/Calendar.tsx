...
const Calendar: React.FC<CalendarProps> = ({
  ...
  const formatDateStringToLocal = (dateString: string): string => {
    ...
    return date ? formatDateToString(date) : dateString;
  };
  ...
  const datesWithTasks = useMemo(() => {
    ...
    tasks.forEach(task => {
      ...
      if (task.startDate && task.dueDate) {
        ...
        const startDate = safeDate(startDateStr);
        const endDate = safeDate(endDateStr);
        
        if (startDate && endDate) {
          const currentDate = safeDate(startDate.getTime());
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 h-fit">
      ...
      <div className="grid grid-cols-7 gap-1 mb-2">
        {['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb'].map((day) => (
          <div key={day} className="p-2 text-center text-xs font-medium text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 rounded">
            {day}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {(() => {
          ...
          for (let i = 0; i < 42; i++) {
            ...
            const completedTasksCount = dayTasks.filter(task => task.status === 'completed').length;
            ...
            days.push(
              <button
                ...
                className={`
                  ...
                  ${
                    isSelected
                      ? 'bg-blue-600 dark:bg-blue-700 text-white border-blue-600 dark:border-blue-700 shadow-md font-semibold hover:bg-blue-700 dark:hover:bg-blue-600'
                      : isToday
                      ? 'bg-blue-100 dark:bg-blue-900/50 border-blue-300 dark:border-blue-600 text-blue-900 dark:text-blue-300 font-semibold hover:bg-blue-200 dark:hover:bg-blue-800/50'
                      : isCurrentMonth
                      ? 'text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 hover:bg-blue-50 dark:hover:bg-gray-700'
                      : 'text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700 border border-gray-100 dark:border-gray-600 hover:bg-blue-50 dark:hover:bg-gray-600'
                  }
                `}
              >
                    ...
                    <div className={`
                      ...
                      ${
                        isSelected
                          ...
                          ? tasksCount > 3
                            ? 'bg-red-300 dark:bg-red-400 opacity-60'
                            : tasksCount > 1
                            ? 'bg-yellow-300 dark:bg-yellow-400 opacity-60'
                            : 'bg-green-300 dark:bg-green-400 opacity-60'
                          : tasksCount > 3
