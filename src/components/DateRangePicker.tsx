...
import { safeDate, safeNow, isSameDay } from '../utils/dateUtils'
...
export const DateRangePicker: React.FC<DateRangePickerProps> = ({
  ...
  const [currentMonth, setCurrentMonth] = useState(() => safeNow().getMonth())
  const [currentYear, setCurrentYear] = useState(() => safeNow().getFullYear())
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-7 gap-0 border-r">
                {firstMonthDays.map((day, index) => {
                  ...
                  const isToday = isSameDay(day, safeNow())
                  ...
                  const now = safeNow()
                  const isPastDate = day < safeDate(now.setHours(0, 0, 0, 0))
                  
                  return (
                    <button
                      key={`first-${index}`}
                      type="button"
                      onClick={() => handleDateClick(safeDate(day.getTime()) || day)}
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-7 gap-0">
                {secondMonthDays.map((day, index) => {
                  ...
                  const isInRange = isDateInRange(day, tempStartDate, tempEndDate)
                  const now2 = safeNow()
                  const isPastDate = day < safeDate(now2.setHours(0, 0, 0, 0))
